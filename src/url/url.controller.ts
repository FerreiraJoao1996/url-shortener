...
export class UrlController {
	...
	async update(@Request() request, @Headers() headers: Record<string, any>) {
		await this.urlValidator.url(request);
		const newShortUrl = await this.urlService.update(request.params, headers['authorization'] ?? null);
		if (newShortUrl) {
			return newShortUrl;
		}
	}
	...
	async find(@Request() request, @Headers() headers: Record<string, any>) {
		await this.urlValidator.url(request);
		const user = await this.urlService.find(request.params.url, headers['authorization'] ?? null);
		if (user) {
			return user;
		}
	}
	...
	async delete(@Request() request, @Headers() headers: Record<string, any>) {
		await this.urlValidator.url(request);
		return await this.urlService.delete(request.params.url, headers['authorization'] ?? null);
	}
